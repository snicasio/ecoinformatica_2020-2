...
####trans<-function(x){
    #    transf<-ifelse(norm_tab$normal == "SI", "NADA",
     #              ifelse (norm_tab$sesgo > 0 & norm_tab$sesgo <= 1, sqrt(x),
      #                     ifelse(norm_tab$sesgo > 1, log(x),
       #                           ifelse(norm_tab$sesgo < 0 & norm_tab$sesgo >= -1, x^2,
        #                                 ifelse(norm_tab$sesgo < -1, exp(x), "NONE")))))
    
    #return(data.frame(transf))
#}
# trans(norm_tab$sesgo)
...
summary(modelo)
anova(modelo)
plot(bio~arvi, data= ind_tab)
abline(modelo, lwd = 2, col="red")
abline(h=mean(ind_tab$bio), lty = 2, col="darkgreen")
